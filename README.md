...
基于 Plotly 的个人数据分析桌面工具，支持数据导入、清洗与变换、可视化图表构建、交互式 Dashboard、不可变数据快照与 AI 辅助分析。当前版本：**0.7.0**。
...
- [ ] **v0.8.1 统计工具箱（一）**：相关性矩阵、线性回归与结果解释。
- [ ] **v0.8.2 统计工具箱（二）**：差异检验、置信区间与结果解释。
- [ ] **v0.9.0 SQL 查询工作台**：编辑器、查询历史、执行计划与结果快照。
