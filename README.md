...
基于 Plotly 的个人数据分析桌面工具，支持数据导入、清洗与变换、可视化图表构建、交互式 Dashboard、不可变数据快照与 AI 辅助分析。当前版本：**1.1.7**。
...
- [x] **v1.0.0 分析故事模式**：将数据来源、清洗步骤、图表、洞察与结论编排成报告。
- [x] **v1.0.1 问答数据概览**：上下文补全行数 / 列数 / count / missing，概览作为确定性证据。
- [x] **v1.1.0 问答工具调用**：LLM 按需调用 row_count / column_stats / distinct_count 工具，失败回退静态概览。
...
- [x] 分析故事模式：将数据来源、清洗步骤、图表、洞察与结论编排成报告（v1.0.0）
