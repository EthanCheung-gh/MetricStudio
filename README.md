...
基于 Plotly 的个人数据分析桌面工具，支持数据导入、清洗与变换、可视化图表构建、交互式 Dashboard、不可变数据快照与 AI 辅助分析。当前版本：**0.6.1**。
...
- [ ] **v0.6.2 Dashboard 编辑增强（二）**：批量布局操作与 Dashboard 撤销重做。
...
- [ ] Dashboard 编辑增强：批量布局、Dashboard 撤销重做
- [x] Dashboard 编辑增强：编辑 / 查看模式、卡片锁定与 Dashboard 复制（v0.6.1）
- [ ] Dashboard 导出：自包含交互式 HTML、PDF / 图片与筛选条件记录
- [ ] 数据源刷新：自动刷新、变更提示、失败恢复与数据版本标记
- [ ] 变换链增强：中间步骤预览、启用 / 禁用、配方复用与导出 SQL / Python
- [ ] 项目可靠性：自动保存、崩溃恢复、项目版本历史与分析上下文复现

### P2：专业分析能力

- [ ] 时间序列工作台：同比 / 环比、移动平均、异常检测与基础预测
- [ ] 统计工具箱：相关性、回归、差异检验、置信区间与结果解释
- [ ] SQL 查询工作台：编辑器、自动补全、查询历史、执行计划与结果快照
