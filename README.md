...
- [x] 数据视图：表格（排序 / 分页）/ CSV、xlsx（多 sheet 合并）、SQLite 文件导入 / 粘贴文本 / 示例数据 / 数据集列表 / SQL 工作台入口
- [x] 数据页四面板（右侧图标栏单开，对齐 main DataView）：清洗（质量扫描 + 安全修复计划 + 配方）、洞察（环比时序 / 叙述 / 五类分析卡）、统计（相关矩阵 / 回归 / 差异检验 / 置信区间）、变换（含表达式计算列与 join）、列统计
