...
def generate_insights(df: pd.DataFrame, max_insights: int = 8) -> list[dict[str, Any]]:
        ...
        if len(monthly) >= 2:
            ...
            if abs(start) > 1e-12:
                ...
                insights.append({
                    "type": "trend",
                    "text": (
                        f"Mean {ncol} {direction} {abs(pct):.0f}% "
                        f"from {monthly.index[0]} ({start:.1f}) to {monthly.index[-1]} ({end:.1f})"
                    ),
                    "evidence": {
                        "metric": "monthly_mean",
                        "field": ncol,
    ...
    for col in nominal:
        ...
        if share >= 0.4:
            insights.append({
                "type": "concentration",
                "text": f"{share * 100:.0f}% of rows belong to “{top}” in {col}",
    ...
    for col in numeric:
        ...
        if ratio >= 0.3:
            direction = "above" if mean > med else "below"
            insights.append({
                "type": "skew",
                "text": (
                    f"Mean {col} ({mean:.1f}) sits {direction} the median ({med:.1f}) "
                    f"— distribution is {direction} the center"
                ),
    ...
    if len(numeric) >= 2:
        ...
        if pairs:
            ...
            insights.append({
                "type": "correlation",
                "text": f"{a} and {b} are strongly correlated (|r| = {v:.2f})",
    ...
    if worst is not None and missing_ratio[worst] > 0:
        ...
        insights.append({
            "type": "missing",
            "text": f"{worst} has {pct}% missing values",
